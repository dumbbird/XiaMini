...
目前只支持音乐库的"收藏的歌曲"和"收藏的专辑"；小组讨论区和话题（不支持小组主页）。
如有意见或建议的请通过"使用说明"里的方式反馈或提出。
*/

// 判断当前网页是否需要本功能
var str = window.location.href;
var strRE = new RegExp("/space/lib-song|/space/lib-album|.com/group");
...
case truststr == "/space/lib-song": {
		num = 25;
		page = number / num;
		pagenumber = Math.ceil(page);
		break;
	}
case truststr == "/space/lib-album": {
	num = 15;
	page = number / num;
	pagenumber = Math.ceil(page);
	break;
}
case truststr == ".com/group": {
		num = 20;
		page = number / num;
